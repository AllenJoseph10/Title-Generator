...
- ~~**`scripts/eval.ts` + `EVAL.md`** (deliverable 4)~~ — **DONE 2026-08-04.** Headline: Spearman **0.259** (mean, 5-fold x 5 repeats, n=172), a real but modest positive signal above the shuffled null. (Was 0.232 until `FAMILY_PRIOR_BLEND` was changed 0.3 → 0 on the strength of this harness: the family term never helped and usually hurt, monotonically across a four-point paired sweep and in every seed and ground-truth configuration tested. The shipped prior is now purely the neighbour mean.) Product-facing figure: slate precision@5 is 0.585 against a 0.500 random baseline (0.571 before the blend change) — a modest edge, and a proxy measured on held-out *corpus* titles, not a before/after measurement of the Task 6 display change (which touched only the app's sort/slice, not the prior, retrieval, or corpus — re-running the eval after that change returns identical numbers by construction). Full detail and caveats in `EVAL.md`.
